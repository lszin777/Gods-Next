...
import { ChevronLeft, Calendar as CalendarIcon, BookOpen, Search, Flame, Edit2, Trash2, Check, X, Smile } from 'lucide-react';

// Importações do Firebase
import { collection, query, where, getDocs, doc, updateDoc, deleteDoc } from 'firebase/firestore';
import { db } from '../firebase';

// Configuração dos filtros com os nomes exatos solicitados
const moodConfig = {
...
export default function DiaryHistory() {
  ...
  // Estado para o filtro de sentimento selecionado (null significa "Todos")
  const [selectedMood, setSelectedMood] = useState(null);
  
  // Estados para o sistema de edição
  const [editingId, setEditingId] = useState(null);
  const [editText, setEditText] = useState("");

  // Pega o ID correto do usuário autenticado (ou usa o seu de teste do banco se não achar)
  const userId = localStorage.getItem('user_id') || 'O7bDCY0Y4wXBBEuQzttKLblerco2';

  const fetchAllDiaries = async () => {
    setLoading(true);
    try {
      // AJUSTADO: Apontando para a coleção correta 'diary_records' que está no seu Firebase
      const q = query(
      ...
      // AJUSTADO: Ordenando pelo campo 'dateKey' existente no seu banco
  ...
  // AJUSTADO: Atualiza no Firebase usando o campo 'diary'
  ...
  const startEditing = (record) => {
    ...
    setEditText(record.diary); // AJUSTADO para mapear o campo 'diary'
  };

  // Formata a data vinda do 'dateKey' (AAAA-MM-DD)
  ...
  // Função para normalizar strings para comparação perfeita (ignora acentos e maiúsculas)
  ...
  // FILTRAGEM DUPLA: Por termo de busca E por sentimento selecionado
  const filteredRecords = records.filter(record => {
    // AJUSTADO: Buscando no campo 'diary' ao invés de 'text'
    const matchesSearch = 
    ...
    // Se o filtro estiver em 'Todos', passa tudo. Caso contrário, compara o record.mood normalizado.
      ...
      <div className="w-full max-w-4xl space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[28px] shadow-sm border border-gray-100">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 text-sm font-semibold text-[#3B429F] bg-indigo-50 hover:bg-indigo-100/80 px-4 py-2.5 rounded-xl transition-colors"
          >
            ...
            {Object.entries(moodConfig).map(([key, config]) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 border ${
                  selectedMood === key 
                    ? 'bg-[#3B429F] text-white border-[#3B429F] shadow-sm' 
        ...
        {loading ? (
          ...
          <div className="relative border-l-2 border-[#3B429F]/10 ml-4 md:ml-6 pl-6 md:pl-8 space-y-8">
            {filteredRecords.map((record, index) => {
              // Mapeia o mood normalizando para achar na nossa lista visual de moods
              ...
              return (
                <motion.div
                  ...
                  className="relative bg-white rounded-[32px] p-6 md:p-8 shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-12 gap-6 items-start group"
                >
                  ...
                  <div className="md:col-span-3 flex md:flex-col items-start gap-3 border-b md:border-b-0 md:border-r border-gray-100 pb-4 md:pb-0 md:pr-4 w-full">
                    <div className="flex md:flex-col items-center md:items-start gap-2 w-full">
                      ...
                      <p className="text-sm font-bold text-gray-800 leading-tight">
                        {formatDateString(record.dateKey)} {/* AJUSTADO para dateKey */}
                      </p>
                    </div>

                    {/* Mostra a tag do sentimento se ele existir */}
                    {currentMood && (
                  ...
                  {/* Conteúdo da Reflexão à Direita */}
                  <div className="md:col-span-9 space-y-4 w-full relative">
                    
                    {/* BOTÕES DE EDICAO E EXCLUSÃO */}
                    {editingId !== record.id && (
                      <div className="absolute top-0 right-0 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white pl-2">
                        <button
                          ...
                          className="p-1.5 text-gray-400 hover:text-[#3B429F] hover:bg-indigo-50 rounded-lg transition-colors"
                          title="Editar"
                        >
                      ...
                      {editingId === record.id ? (
                        <div className="space-y-2 mt-2">
                          <textarea
                            ...
                            className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#3B429F] text-gray-700 text-sm leading-relaxed font-light italic resize-y"
                            rows={4}
                          />
                          <div className="flex justify-end gap-2">
                            <button
                              onClick={() => setEditingId(null)}
                              className="inline-flex items-center gap-1 text-xs font-semibold text-gray-500 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition-colors"
                            >
                              <X className="w-3.5 h-3.5" /> Cancelar
                            </button>
                            <button
                              onClick={() => handleSaveEdit(record.id)}
                              className="inline-flex items-center gap-1 text-xs font-semibold text-white bg-[#3B429F] hover:bg-[#2D3380] px-3 py-1.5 rounded-lg transition-colors"
                            >
                        ...
                        <p className="text-gray-700 text-base md:text-lg leading-relaxed whitespace-pre-wrap font-light italic pr-12">
                          "{record.diary}" {/* AJUSTADO para usar o campo diary */}
                        </p>
                      )}
                    </div>
                    ...
                    {(record.verse || record.reference) && (
                      <div className="bg-[#EFEBE4]/50 border border-[#E6E0D5] rounded-2xl p-4">
                        ...
                        <span className="text-xs text-gray-500 font-bold block mt-1 text-right">
                          — {record.reference || "Sem referência"}
                        </span>
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-[32px] p-12 text-center border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-400 font-light text-sm max-w-sm mx-auto">
              Não encontramos nenhuma anotação correspondente aos filtros aplicados. Tente selecionar outra opção ou palavra-chave.
            </p>
          </div>
        )}
